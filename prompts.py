...
def get_la_permitting_system_prompt(existing_names: list[str] = None) -> str:
    ...
    prompt = (
        ...
        "Search these specific Los Angeles municipal data sources for development projects in "
        "early stages that are likely to involve art commissioning. Focus on private developments "
        "with permit valuations above $5M and any public capital project with a percent-for-art "
        "requirement.\n\n"

        "Sources to search:\n"
        "- LA Department of Building and Safety (LADBS): recent building permit applications "
        "for new construction or major renovation with valuations above $5M\n"
        "- LA Department of City Planning: cases in the planning pipeline — environmental impact "
        "reports (EIRs), specific plan amendments, zone changes, conditional use permits for "
        "large developments\n"
        "- LA City Planning Commission and Area Planning Commission agendas: recent meeting "
        "agendas listing major project hearings\n"
        "- LA County Arts Commission: active or upcoming civic art projects, percent-for-art "
        "calls, and RFQs\n"
        "- LA Metro: capital projects in planning or design phases with art commissioning "
        "requirements\n"
...
def get_la_trade_press_system_prompt(existing_names: list[str] = None) -> str:
    """
    Fallback prompt used when the municipal-source search returns fewer than
    LA_FALLBACK_THRESHOLD leads. Focuses exclusively on trade press and developer
    announcements rather than trying to search municipal databases.
    """
    today = date.today().strftime("%B %d, %Y")
    ...
    prompt = (
        ...
        "Search Los Angeles real estate trade press and developer announcements for early-stage "
        "development projects that are likely to involve art commissioning. Focus on projects "
        "that have NOT yet broken ground — in planning, entitlement, or permitting.\n\n"

        "Sources to search:\n"
        "- Urbanize LA: coverage of new development proposals, entitlements, and planning approvals\n"
        "- The Real Deal Los Angeles: commercial and mixed-use development news\n"
        "- Bisnow Los Angeles: commercial real estate development pipeline\n"
        "- Developer and architecture firm press releases about upcoming LA projects\n"
        "- Coverage of LA Metro, transit, or civic construction projects in design or planning\n"
        "- News coverage of recent EIR approvals, entitlement hearings, or groundbreaking "
        "announcements for major developments\n\n"
        ...
        "- estimated_budget: best estimate as a range, e.g. \"$150K–$500K\"\n"
        "    Private development: typically 0.5%–2% of construction cost\n"
        "    Public projects: typically 1%–2% per applicable ordinance\n"
        "    Do not leave blank — estimate even when confidence is Low\n"
        "- budget_basis: one sentence explaining the derivation\n"
        "- budget_confidence: exactly one of \"High\", \"Medium\", or \"Low\"\n"
        "- project_stage: exactly one of \"Planning/Entitlement\", \"Permitted\", "
        "\"Design Phase\", \"Under Construction\", \"Near Completion\", or \"Unknown\"\n"
        "- lead_source: the specific publication or source, e.g. \"Urbanize LA\", "
        "\"The Real Deal LA\", \"Bisnow LA\", \"Developer Announcement\"\n"
        "- notes: anything else relevant\n\n"
